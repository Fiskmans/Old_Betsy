...
namespace engine
{
	size_t GraphicsFramework::ourTotalUsage = 0;
	std::unordered_map<std::string, std::pair<size_t, std::vector<GraphicsFramework::memoryUser>>> GraphicsFramework::ourMemoryUsers;

	const std::array<float, DXGI_FORMAT_V408 + 1> GraphicsFramework::FormatToSizeLookup =
	{
	...
	bool GraphicsFramework::Init()
	{
		...
		tools::Stopwatch watch;
		{
			...
			tools::V2ui size = WindowManager::GetInstance().GetSize();
			AddGraphicsMemoryUsage(static_cast<size_t>(GraphicsFramework::FormatToSizeLookup[DXGI_FORMAT_R8G8B8A8_UNORM] * size[0] * size[1]), "Backbuffer", "Engine");
		}
	...
	void GraphicsFramework::AddGraphicsMemoryUsage(size_t aAmount, const std::string& aName, const std::string& aCategory)
	{
#if TRACKGFXMEM
