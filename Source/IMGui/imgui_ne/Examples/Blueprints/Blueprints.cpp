            ...
            builder.Begin(node.ID);
                ...
                for (auto& output : node.Outputs)
                {
                    ...
                    if (output.Type == PinType::String)
                    {
						...
						const char* listbox_items[] = { "Select", "Bool", "Int", "Float", "String" };
						int listbox_item_current = 1;
						if (listbox_item_current > 4)
						{
							listbox_item_current = 0;
						}
						ImGui::Combo("##edit", &listbox_item_current, listbox_items, IM_ARRAYSIZE(listbox_items), 4);
