...
PixelOutput pixelShader(VertexToPixel input)
{
	...
	float mask_matR = MaterialMap.Sample(defaultSampler, ((input.myUV * 1) + (objectLifeTime * .02))).r;
	float mask_matG = MaterialMap.Sample(defaultSampler, ((input.myUV * 1) + (objectLifeTime * .1))).g; //perlinNoise
	float mask_matB = MaterialMap.Sample(defaultSampler, ((input.myUV * 1) + (objectLifeTime * .01))).b;
