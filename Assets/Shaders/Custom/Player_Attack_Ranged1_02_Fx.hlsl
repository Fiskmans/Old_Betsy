...
#include "Data/Shaders/Sampling.hlsli"
	...
	float blueChannelMask = MaterialMap.Sample(MeshTextureSampler, input.myUV).b;
	float blueChannelColor = blueChannelMask * 0.3;


	float onInputTriggerTime = totalTime - objectLifeTime;
	blueChannelColor = lerp(blueChannelColor, blueChannelColor * 10, 1.2 - (cos((objectLifeTime) * 6) + 1) / 2);


	float3 EmissiveRGB = float3(blueChannelColor, 0, 0);
