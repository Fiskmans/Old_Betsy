...
#include "Data/Shaders/Sampling.hlsli"
	...
	if (lightOverTime > 0.5)
		lightAmount = (lightOverTime);

	if (intensityCameraFresnel > 0.9999 * innerGlowPulse)
		colorCamera = float4(0.0, 0.0 , 0.0 , 0.0);

	else if (intensityCameraFresnel > 0.9 * middleShadowPulse)
		colorCamera = float4(0.0, 0.0, 0.0,  0.0);

	else if (intensityCameraFresnel > 0.425 * middleGlowPulse)
		colorCamera = float4(0.0, 0.0, 0.0, 0.1 * lightAmount);

	else if (intensityCameraFresnel > 0.3 * outerGlowPulse)
		colorCamera = float4(0.0, 0.0, 0.0, 0.0025 * lightAmount);
