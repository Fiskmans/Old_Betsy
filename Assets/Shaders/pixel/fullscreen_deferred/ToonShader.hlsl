...
Texture2D PerlinNoise : register(t10);
...
{
    float perlin1 = PerlinNoise.Sample(defaultSampler,float2(20.232,15.25) + aGroundPosition.xz / 150000 + float2(time / 2000, 0)).r;
    float perlin2 = PerlinNoise.Sample(defaultSampler, float2(20.232, 15.25) + aGroundPosition.xz / 150000 + float2(0, time / 200000)).r;
    float perlin = (perlin1 + perlin2) / 2;
    if (perlin > cloudIntensity)
    {
        return 0;
    }
    return clamp(0, 0.35, (cloudIntensity - perlin) * 3.5 / cloudIntensity);
}
...
PixelOutput pixelShader(VertexToPixel input)
{
	...
	float intensity = dot(normalize(environmentLightDirection.xyz), normalize(normal));
	if (intensity < 0)
		intensity = 0;
